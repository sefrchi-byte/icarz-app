...
try{
  ...
  $db->exec("CREATE TABLE IF NOT EXISTS cmd_vehicles (id INT AUTO_INCREMENT PRIMARY KEY,plate VARCHAR(30) NOT NULL UNIQUE,car VARCHAR(120) NOT NULL DEFAULT '',km INT NOT NULL DEFAULT 0,market INT NOT NULL DEFAULT 0,delta INT NOT NULL DEFAULT 0,body_score INT NOT NULL DEFAULT 0,last_claim_days INT NOT NULL DEFAULT 0,note VARCHAR(255) NOT NULL DEFAULT '',updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  $db->exec("CREATE TABLE IF NOT EXISTS cmd_claims (id INT AUTO_INCREMENT PRIMARY KEY,vehicle_id INT NOT NULL,ins_date VARCHAR(20) NOT NULL DEFAULT '',insurer VARCHAR(120) NOT NULL DEFAULT '',note VARCHAR(255) NOT NULL DEFAULT '',amount INT NOT NULL DEFAULT 0,INDEX (vehicle_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  $db->exec("CREATE TABLE IF NOT EXISTS cmd_owners (id INT AUTO_INCREMENT PRIMARY KEY,vehicle_id INT NOT NULL,seq TINYINT NOT NULL DEFAULT 1,own_date VARCHAR(20) NOT NULL DEFAULT '',note VARCHAR(255) NOT NULL DEFAULT '',km_at INT NOT NULL DEFAULT 0,active TINYINT NOT NULL DEFAULT 0,INDEX (vehicle_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  $db->exec("CREATE TABLE IF NOT EXISTS cmd_panels (id INT AUTO_INCREMENT PRIMARY KEY,vehicle_id INT NOT NULL,panel VARCHAR(120) NOT NULL DEFAULT '',kind VARCHAR(12) NOT NULL DEFAULT 'orig',INDEX (vehicle_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  ...
  if(!$db->query('SELECT COUNT(*) c FROM cmd_vehicles')->fetch()['c']){
    $st=$db->prepare('INSERT INTO cmd_vehicles (id,plate,car,km,market,delta,body_score,last_claim_days,note) VALUES (?,?,?,?,?,?,?,?,?)');
    foreach($seedCmdVehicles as $r) $st->execute([$r['id'],$r['plate'],$r['car'],$r['km'],$r['market'],$r['delta'],$r['body_score'],$r['last_claim_days'],$r['note']]);
    $st=$db->prepare('INSERT INTO cmd_claims (vehicle_id,ins_date,insurer,note,amount) VALUES (?,?,?,?,?)');
    foreach($seedCmdClaims as $r) $st->execute([$r['vehicle_id'],$r['ins_date'],$r['insurer'],$r['note'],$r['amount']]);
    $st=$db->prepare('INSERT INTO cmd_owners (vehicle_id,seq,own_date,note,km_at,active) VALUES (?,?,?,?,?,?)');
    foreach($seedCmdOwners as $r) $st->execute([$r['vehicle_id'],$r['seq'],$r['own_date'],$r['note'],$r['km_at'],$r['active']]);
    $st=$db->prepare('INSERT INTO cmd_panels (vehicle_id,panel,kind) VALUES (?,?,?)');
    foreach($seedCmdPanels as $r) $st->execute([$r['vehicle_id'],$r['panel'],$r['kind']]);
    $log[]=count($seedCmdVehicles).' خودرو + قبض‌ها و مالکیت و پنل‌ها ثبت شد';
  } else $log[]='داده‌های هوش خودرو قبلاً ثبت شده‌اند';
